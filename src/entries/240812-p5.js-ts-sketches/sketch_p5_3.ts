...
function mySketch3(p: typeof p5) {
    let angle: number = 0;
    let canvas: any;
    ...
    p.draw = () => {
        ...
        p.noFill();
        p.beginShape();
        for (let x: number = 0; x < p.width; x += 10) {
            let y: number = p.map(p.sin(angle + x * 0.1), -1, 1, 0, p.height);
            p.vertex(x, y);
        }
        p.endShape();
        angle += 0.1;
    ...
    p.mousePressed = () => {
        if (p.mouseX > 0 && p.mouseX < p.width && p.mouseY > 0 && p.mouseY < p.height) {
            canvas?.elt.focus(); // Use optional chaining to safely access the element
            return false;
        }
    };
}
