...
const d3testId: string = "d3-chart-circle";
 ...
 */
function createBarChart(containerId: string = d3testId): void {
    const data: number[] = [30, 80, 45, 60, 20, 90, 50];

    // Select the container and append an SVG element
    const svg: d3.Selection<SVGSVGElement, unknown, HTMLElement, any> = d3.select<HTMLElement, unknown>(`#${containerId}`)
        .append("svg")
        .attr("width", 500)
        .attr("height", 300);
 ...
 */
function createSimpleCircle(containerId: string = d3testId): void {
